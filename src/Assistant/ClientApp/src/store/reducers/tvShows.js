...
const reducer = (state = initialState, action) => {
    switch (action.type) {
        case actionTypes.SET_TVSHOWS:
            return setTvShows(state, action);
        case actionTypes.FETCH_TVSHOWS_STARTED:
            return fetchTvShowsStarted(state);
        case actionTypes.FETCH_TVSHOWS_FAILED:
            return fetchTvShowsFailed(state);
        case actionTypes.SUBMIT_TVSHOW:
            return submitTvShow(state, action);
        case actionTypes.SUBMIT_TVSHOW_STARTED:
            return submitTvShowsStarted(state);
        case actionTypes.SUBMIT_TVSHOW_FAILED:
            return submitTvShowsFailed(state);
        case actionTypes.DELETE_TVSHOW:
            return deleteTvShow(state, action);
        case actionTypes.DELETE_TVSHOW_STARTED:
            return deleteTvShowsStarted(state);
        case actionTypes.DELETE_TVSHOW_FAILED:
            return deleteTvShowsFailed(state);
        default:
            return state;
    }
};
